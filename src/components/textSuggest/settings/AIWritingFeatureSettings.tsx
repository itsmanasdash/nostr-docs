  ...
  return (
    <>
      ...
      >
        ...
        <Typography id="proofread-document-heading" variant="subtitle2" sx={{ mb: 0.5 }}>
          Proofread document
        </Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: "block", mb: 1.5 }}>
            Tell the local model what to do. It will review the complete document,
            then show a diff you can accept or reject. Use “Fix spelling,
            grammar, and punctuation” for autocorrection without changing your
            tone or Markdown formatting.
          </Typography>
          <TextField
            fullWidth
            multiline
            ...
            value={instruction}
            slotProps={{ htmlInput: { maxLength: MAX_PROOFREAD_INSTRUCTION_CHARS } }}
            disabled={running}
          ...
          {!hasModel && (
            <Alert severity="info" sx={{ mt: 1.5 }}>
              Choose a downloaded GGUF model below before proofreading.
            </Alert>
          )}
          {documentLength === 0 && (
          ...
          {documentTooLong && (
            <Alert severity="warning" sx={{ mt: 1.5 }}>
              This document is over the {MAX_PROOFREAD_DOCUMENT_CHARS.toLocaleString()}-character
              limit for one local proofreading pass. Shorten it before reviewing;
              the document will never be silently truncated.
            </Alert>
          )}
          {(submitError || (proofreadStatus.kind === "error" && proofreadStatus.message)) && (
